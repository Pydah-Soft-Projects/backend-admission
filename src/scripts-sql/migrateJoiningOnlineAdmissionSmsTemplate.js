/**
 * Idempotent migration: adds Communications template for "Send admission SMS" (joining desk).
...
const TEMPLATE_NAME = 'Joining · online admission link';

/** Strip full-line -- comments; split on semicolons (no semicolons inside our INSERT values). */
...
const run = async () => {
  try {
    ...
    const sqlPath = path.join(__dirname, '..', '..', 'sql', 'migrations', '20260502_joining_online_admission_sms_template.sql');
    ...
    console.log(`✓ Applied migration: "${TEMPLATE_NAME}" (update DLT id in Communications if needed).`);
  } catch (e) {
    console.error('migrateJoiningOnlineAdmissionSmsTemplate:', e.message || e);
    process.exitCode = 1;
  } finally {
    await closeDB();
  }
