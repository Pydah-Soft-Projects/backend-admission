...
export const getMismatchedLeadsReport = async (req, res) => {
  try {
    ...
    let query = `
      SELECT id, name, phone, state, district, mandal, needs_manual_update, enquiry_number
      FROM leads 
...
export const getDuplicateLeadsReport = async (req, res) => {
  try {
    ...
    const [leads] = await pool.execute(`
      SELECT id, name, phone, father_phone, alternate_mobile, enquiry_number, father_name, district, mandal, created_at, updated_at, source, lead_status
      FROM leads
      WHERE student_group IN (${groups.map(() => '?').join(',')})
        AND academic_year = ?
    `, [...groups, year]);
    ...
    // Sort phone    // Sort groups by duplicate count DESC
